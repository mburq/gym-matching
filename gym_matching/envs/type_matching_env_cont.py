...
class TypeMatchingEnvContinuous(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(low=-5, high=5, shape=(self.num_types,))
    ...
    def _step(self, action):
        ...
        if np.all(action <= self.state):
            act = np.maximum(action, 0).astype(int)
            (matches, reward) = gurobi_max_weight_matching(self.matrix, act, self.num_types)
            self.state -= matches
        else:
            reward = 0
